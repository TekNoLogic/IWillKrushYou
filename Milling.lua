...
function GetAuctionBuyout(item)
	...
	elseif cpigments[id] then
		-- Common pygments drop an average 3 items per milling
		-- The lowest tier only drops 2.5 items per milling
		local best = getbest(cpigments[id])
		if best then return 5/(id == 39151 and 2.5 or 3)*best end
	end
